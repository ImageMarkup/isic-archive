...
class TaskResource(IsicResource):
    ...
    @describeRoute(Description('Get the current user\'s QC review tasks.').responseClass('Task'))
    @access.user
    def getReviewTasks(self, params):
        ...
        for prereviewFolder in Folder().find(
            ...
            results.append(
                {'dataset': {'_id': dataset['_id'], 'name': dataset['name']}, 'count': count}
            )
    ...
    @describeRoute(
        Description('Redirect to a QC review task.').param(
            'datasetId', 'An ID for the dataset to get a QC review task for.', required=True
        )
    )
    @access.cookie
    @access.user
    ...
    def _pipeline1AllImages(self, user):
        return [
            # Filter viewable images out of all items
            {
                '$match': {
                    'folderId': {
                        '$in': [dataset['folderId'] for dataset in Dataset().list(user=user)]
                    }
                }
            }
        ]
            ...
            # Get only images with no successful expert segmentations
            {
                '$match': {
                    'segmentations.reviews': {
                        '$not': {
                            '$elemMatch': {'skill': Segmentation().Skill.EXPERT, 'approved': True}
                        }
                    }
                }
            }
    ...
    def _pipeline4CountImages(self):
        return [
            ...
            {'$group': {'_id': '$meta.datasetId', 'count': {'$sum': 1}}}
        ]
    ...
    def _pipeline4ListImages(self):
        return [
            ...
            # Group images by dataset id
            {
                '$group': {
                    '_id': '$meta.datasetId',
                    'images': {'$push': {'_id': '$_id', 'name': '$name', 'updated': '$updated'}},
                }
            },
        ]
    ...
    def _pipeline5JoinDataset(self):
        return [
            ...
            {'$sort': {'dataset.name': SortDir.ASCENDING}},
        ]
    ...
    def _pipeline4RandomImage(self):
        return [
            ...
            {'$project': {'_id': 1, 'name': 1, 'updated': 1}},
        ]
    ...
    @describeRoute(
        ...
        .param(
            'details',
            'Whether a full listing of images is returned, or just counts.',
            dataType='boolean',
            default=False,
        )
    )
        ...
        pipeline = list(
            itertools.chain(
                ...
                (
                    ...
                    else self._pipeline3MissingSegmentations()
                ),
                (self._pipeline4ListImages() if details else self._pipeline4CountImages()),
    ...
    @describeRoute(
        ...
        .notes(
            'The image is selected randomly from all those requiring segmentation in the given '
            'dataset.'
        )
    ...
    @describeRoute(
        Description('Redirect to a random segmentation task.').param(
            'datasetId', 'An ID for the dataset to get a segmentation task for.', required=True
        )
    )
    @access.cookie
    @access.user
    ...
    @describeRoute(Description('Get the current user\'s annotation tasks.').responseClass('Task'))
    @access.user
        ...
        results = [
            {
                ...
                'count': Study()
                .childAnnotations(study=study, annotatorUser=user, state=Study().State.ACTIVE)
                .count(),
            }
    ...
    @describeRoute(
        Description('Get the next pending annotation.').param(
            'studyId', 'An ID for the study to get a pending annotation for.', required=True
        )
    )
    @access.user
    ...
    @describeRoute(
        Description('Redirect to the next annotation task.').param(
            'studyId', 'An ID for the study to get an annotation task for.', required=True
        )
    )
    @access.cookie
    @access.user
